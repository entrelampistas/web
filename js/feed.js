...
(function () {
  ...
  /* ── primera pregunta del índice, respondible aquí; el botón sigue en la herramienta ── */
  ...
  var q = datos.dimensiones[0].preguntas[0];
  ...
  var textoQ = document.querySelector('[data-feed-texto]');
  if (textoQ) textoQ.textContent = q.texto;

  function hay() { return elegida !== undefined && elegida !== null; }
  function pintar() {
    zona.innerHTML = '';
    zona.classList.toggle('hay-eleccion', hay());
    ...
    if (pasos) {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        if (i === 0) s.className = hay() ? 'es-respondida' : 'es-actual';
        pasos.appendChild(s);
      }
    }
    if (seguir) {
      seguir.textContent = hay() ? 'seguir con la pregunta 2 →' : 'calcular mi índice';
      seguir.setAttribute('href', hay() ? '/indice#empezar' : '/indice');
    }
  }
  function elegir(i) {
    elegida = i;
