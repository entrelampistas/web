...
- 24-09-2026 · ◆ **Variantes en exploración** (temporal): una página puede traer varias versiones de un bloque en `<div data-solo-v="a|b|c" data-nombre="…">`; `js/variantes.js` elige con `?v=` (recordado en la sesión) y pinta un selector arriba del `main` **solo fuera de producción**; en entrelampistas.com siempre se ve la `a`. Hoy: `/mapas?v=a|b|c` (retícula · lista con foto pequeña · tarjetas con foto grande). Cuando la autora elija, se deja solo la elegida y se retiran las otras (y `variantes.js` si ya no queda ninguna).
