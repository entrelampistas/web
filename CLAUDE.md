...
- 25-09-2026 · **Editorial suiza** del ensayo (`json › editorial: { estilo: "suizo", pregunta: "forma" | "tinta" }`, CSS al final de `styles/tema.css`), en Habitabilidad y El mapa de tu mente; Criterio sigue con la presentación anterior hasta tener su texto final. Una columna en bandera; resumen como entradilla grande sin caja; cada sección abre sobre papel (filete, folio `01 / 05`, título 36) y la foto va después sin texto ni velo, dentro del margen (brief §1); primer párrafo de sección a 19/500; frases destacadas en 700 (◆ `secciones[].destacados`, el build falla si no están en el texto); cita grande sin caja (24 si es larga); listas con filetes y folio colgado (`tipo: "lista"`); cifra de un dato del párrafo (`tipo: "cifras", tras, cifras`). Carácter por mapa: Habitabilidad cuelga ○ ante la pregunta de cierre; El mapa de tu mente saca la pregunta a un bloque de tinta a todo lo ancho. Cierre con el párrafo final de la autora en 21/500.
